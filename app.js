...
function openDetailModal(cardId, title, price, longDesc, imgs, type) {
    ...
    // 🧁 실시간 후기 불러오기 (관리자용 승낙/삭제 버튼 추가 버전)
    ...
    db.collection("reviews").where("cardId", "==", cardId).orderBy("timestamp", "asc").onSnapshot((snapshot) => {
        ...
        snapshot.forEach((doc) => {
            ...
            if (rData.isApproved || isAdmin) {
                ...
                rDiv.style.padding = "8px 0";
                rDiv.style.display = "flex";
                rDiv.style.justifyContent = "space-between";
                rDiv.style.alignItems = "center";
                rDiv.style.borderBottom = "1px dashed #eee";

                const pendingText = (!rData.isApproved && isAdmin) ? " <span style='color: #FF6B6B; font-size: 0.8rem; font-weight:bold;'>[승인 대기]</span>" : "";

                let adminBtnsHtml = "";
                if (isAdmin) {
                    adminBtnsHtml = `
                        <div style="display:flex; gap:5px; flex-shrink: 0;">
                            ${!rData.isApproved ? `<button class="approve-review-btn" data-id="${doc.id}" style="background:#4A2E1B; color:white; border:none; border-radius:4px; padding:3px 8px; font-size:0.75rem; cursor:pointer; font-weight:bold;">승낙</button>` : ''}
                            <button class="delete-review-btn" data-id="${doc.id}" style="background:#FF6B6B; color:white; border:none; border-radius:4px; padding:3px 8px; font-size:0.75rem; cursor:pointer; font-weight:bold;">삭제</button>
                        </div>
                    `;
                }

                rDiv.innerHTML = `
                    <span style="font-size:0.9rem; padding-right:10px; word-break:break-all;">
                        <strong>${rData.isAnonymous && !isAdmin ? "익명" : rData.nickname}:</strong> ${rData.content}${pendingText}
                    </span>
                    ${adminBtnsHtml}
                `;
